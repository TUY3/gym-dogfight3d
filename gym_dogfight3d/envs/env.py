...
class DogFightEnv(gym.Env):
    ...
    def __init__(self, opponent='self-play'):
        ...
        self.dt = 0.0625
        self.ax = None
        assert opponent in ["self-play", "random", "rule-based"], "Invalid opponent, must be either self-play, random, rule-based"
    ...
    def attack(self, uav1: UAV, uav2: UAV):
        attack_reward = 0
        # uav1 attack uav2
        uav1.target_distance = np.linalg.norm(uav2.position - uav1.position)
        uav1_dir = uav1.direction
        tar_dir = normalize(uav2.position - uav1.position)
        uav1.target_angle = math.degrees(math.acos(max(-1, min(1, np.dot(uav1_dir, tar_dir)))))
        if uav1.target_angle < uav1.attack_angle and uav1.attack_range[0] < uav1.target_distance < uav1.attack_range[1]:
            uav2.hit(0.05)
            attack_reward = 0.05
    ...
    def reset(self,
        *,
        ...
        init_postion1 = np.array([random.randint(-init_pos[0], init_pos[0]),
        ...
        init_postion2 = np.array([random.randint(-init_pos[0], init_pos[0]),
                                  random.randint(-init_pos[1], init_pos[1]),
                                  random.randint(-init_pos[2], init_pos[2])], dtype=np.float32)
        ...
        self.current_step = 0
        self.info = {"attack": 0, "be_attacked": 0, "steps": 0}
    ...
    def step(self, action):
        ...
        # if done:
        #     self.info["steps"] = self.current_step
    ...
    def _render(self):
        ...
        plt.legend(labels=[f"{self.uav1.health_level:.3f}\n{self.uav1.pitch_attitude}",
                           f"{self.uav2.health_level:.3f}\n{self.uav2.pitch_attitude}"],
                   loc="upper left",
...
def test():
    env = DogFightEnv(opponent='self-play')
    # obs = env.reset()
